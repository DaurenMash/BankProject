...
public class KafkaGetAllUsersTest {

    private static String jwtToken;

    private static final String KAFKA_BOOTSTRAP_SERVERS = "localhost:9092";
    private static final String TOPIC_GET_ALL_USERS = "user.get.all";
    private static final String TOPIC_GET_ALL_USERS_RESPONSE = "user.get.all.response";
    ...
    private static void loadConfig() {
        ...
        try (InputStream input = KafkaGetAllUsersTest.class.getClassLoader().getResourceAsStream("config.properties")) {
            if (input != null) {
    ...
    public static void main(String[] args) {
        ...
        consumer.subscribe(Collections.singletonList(TOPIC_GET_ALL_USERS_RESPONSE));

        final KafkaRequest request = new KafkaRequest();
        request.setRequestId(UUID.randomUUID().toString());
        request.setJwtToken(jwtToken);

        producer.send(new ProducerRecord<>(TOPIC_GET_ALL_USERS, request));
        producer.flush();
