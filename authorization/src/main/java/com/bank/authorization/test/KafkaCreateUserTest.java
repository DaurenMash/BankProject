...
public class KafkaCreateUserTest {
    ...
    private static String jwtToken = "eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxIiwiYXV0aG9yaXRpZXMiOlsiUk9MRV9BRE1JTiJdLCJpYXQiOjE3NDE4NjgwMjgsImV4cCI6MTc0MTkwNDAyOH0.QjdFSqnLFbCRyB7j-vMCjv4HVHFFiNa2y8COvAddodM";
    ...
    public static void main(String[] args) {
        ...
        userDto.setProfileId(5L);
        userDto.setPassword("password123");

        // Создаем KafkaRequest для отправки
        KafkaRequest request = new KafkaRequest();
        request.setJwtToken(jwtToken); // Вставьте реальный токен
        request.setPayload(userDto); // Добавление UserDto в payload
        ...
        for (ConsumerRecord<String, KafkaResponse> consumerRecord : records) {
            ...
            if (response.getRequestId().equals(request.getJwtToken())) { // Сравниваем requestId с JWT-токеном
